...
public class TopClass implements ActionListener, KeyListener {
	...
	private JFrame f = new JFrame("Flappy Bird Game");
	...
	private JPanel createContentPane() {
		...
		startGame.setAlignmentX(1.5f); //center horizontally on-screen
		startGame.setAlignmentY(0.5f); //center vertically on-screen
		startGame.addActionListener(this);
		topPanel.add(startGame);
		
		playMessage = new JLabel("Use Spacebar to Jump");
		playMessage.setForeground(Color.RED);
		playMessage.setFont(new Font("Calibri", Font.BOLD, 30));
		playMessage.setAlignmentX(0.0f); //center horizontally on-screen
		playMessage.setAlignmentY(0.5f); //center vertically on-screen
		topPanel.add(playMessage);
	...
	private void collisionDetection(BottomPipe bp1, BottomPipe bp2, TopPipe tp1, TopPipe tp2, Bird bird) {
		...
		if(bird.getY() + BIRD_HEIGHT > SCREEN_HEIGHT*7/8) { //ground detection
			pgs.sendText("Press 'B' to Restart");
			loopVar = false;
